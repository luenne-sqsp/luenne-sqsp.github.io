const splitCarousel = document.getElementById('split-carousel');
const splitIndicators = document.querySelectorAll('#split-indicators button');

let originalSlides = Array.from(document.querySelectorAll('.split-slide'));
...
splitCarousel.appendChild(firstClone);
splitCarousel.insertBefore(lastClone, originalSlides[0]);

let allSlides = document.querySelectorAll('.split-slide');

// init position
splitCarousel.style.transform = `translateX(-${currentIndex * allSlides[0].offsetWidth}px)`;
...
function updateSlides(animate = true) {
  ...
  splitCarousel.style.transition = animate ? 'transform 0.6s ease' : 'none';
  splitCarousel.style.transform = `translateX(-${currentIndex * slideWidth}px)`;
  ...
  // Update indicators
  splitIndicators.forEach(btn => btn.classList.remove('active'));
  if (currentIndex >= 1 && currentIndex <= originalSlides.length) {
    splitIndicators[getRealIndex(currentIndex)]?.classList.add('active');
  }

  splitCarousel.addEventListener('transitionend', handleLoopEdges);
}

function handleLoopEdges() {
  splitCarousel.removeEventListener('transitionend', handleLoopEdges);

  if (allSlides[currentIndex].classList.contains('clone')) {
    const slideWidth = allSlides[0].offsetWidth;

    splitCarousel.style.transition = 'none';
...
splitIndicators.forEach((button, btnIndex) => {
...
splitCarousel.addEventListener('mouseenter', () => clearTimeout(autoPlayTimeout));
splitCarousel.addEventListener('mouseleave', () => startAutoPlay());
